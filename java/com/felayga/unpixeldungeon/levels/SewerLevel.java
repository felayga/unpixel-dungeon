...
public class SewerLevel extends RegularLevel {
    ...
    protected void decorate() {
        ...
        for (int i = 0; i < WIDTH; i++) {
            if (map(i) == Terrain.WALL && map(i + WIDTH) == Terrain.PUDDLE && Random.Int(4) == 0) {
                map(i, Terrain.WALL_DECO);
            }
        }

        for (int i = WIDTH; i < LENGTH - WIDTH; i++) {
            if (map(i) == Terrain.WALL && map(i - WIDTH) == Terrain.WALL && map(i + WIDTH) == Terrain.PUDDLE && Random.Int(2) == 0) {
                map(i, Terrain.WALL_DECO);
            }
        }
        ...
        for (int i = WIDTH + 1; i < LENGTH - WIDTH - 1; i++) {
            if (map(i) == Terrain.EMPTY) {
                int count = 0;
                count += map(i + 1) == Terrain.WALL ? 1 : 0;
                count += map(i - 1) == Terrain.WALL ? 1 : 0;
                count += map(i + WIDTH) == Terrain.WALL ? 1 : 0;
                count += map(i - WIDTH) == Terrain.WALL ? 1 : 0;
