...
public class MagicMissile extends Emitter {
    ...
    public static void invisibility(Group group, int from, int to, Callback callback) {
        MagicMissile missile = ((MagicMissile) group.recycle(MagicMissile.class));
        missile.reset(from, to, callback);
        missile.size(4);
        missile.pour(InvisibilityParticle.FACTORY, 0.01f);
    }

    public static void haste(Group group, int from, int to, Callback callback) {
        MagicMissile missile = ((MagicMissile) group.recycle(MagicMissile.class));
        missile.reset(from, to, callback);
        missile.size(4);
        missile.pour(HasteParticle.FACTORY, 0.01f);
    }
    ...
    public static class InvisibilityParticle extends PixelParticle.Shrinking {
        public static final Emitter.Factory FACTORY = new Factory() {
            @Override
            public void emit(Emitter emitter, int index, float x, float y) {
                ((InvisibilityParticle) emitter.recycle(InvisibilityParticle.class)).reset(x, y);
            }
    ...
    public static class HasteParticle extends PixelParticle.Shrinking {
        public static final Emitter.Factory FACTORY = new Factory() {
            @Override
            public void emit(Emitter emitter, int index, float x, float y) {
                ((HasteParticle) emitter.recycle(HasteParticle.class)).reset(x, y);
            }
